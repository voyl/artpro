...
namespace ArtTechIVRUtils
{
    public partial class AgentStats : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                txtTarih.Text = DateTime.Today.ToString("dd.MM.yyyy");
                txtTarih2.Text = DateTime.Today.ToString("dd.MM.yyyy");
               LoadAgentData();
            //    LoadAgentSkills(Request.QueryString["agentId"].ToString());
        ...
        public void LoadAgentData()
        {
            ...
            else
            {
                ...
                System.Data.SqlClient.SqlCommand sqlCmd = new System.Data.SqlClient.SqlCommand("select MAX(LASTUPDATE) as LASTUPDATE2, SUM(cast(ACDCALLS as int)) as ACDCALLS2, SUM(cast(HOLDCALLS as int)) as HOLDCALLS2, SUM(cast(TACDTIME as int)) as TACDTIME2, SUM(cast(THOLDTIME as int)) as THOLDTIME2 , sum(cast ( TACWTIME as int)) as TACWTIME2, sum(cast ( TACWCALLTIME as int)) as TACWCALLTIME2,  "
                    ...
                    + " from AGENTSTATS WHERE LOGID='" + Request.QueryString["agentId"] + "' AND convert(datetime, ROW_DATE, 104) between convert(datetime, '" + txtTarih.Text + "',104) and convert(datetime, '" + txtTarih2.Text + "', 104)", _sqlConn);
                //select * from AGENTSTATS WHERE LOGID=" + Request.QueryString["agentId"] + " AND ROW_DATE='" + txtTarih.Text+ "'", _sqlConn);
                _sqlConn.Open();
                ...
                }
                {
                    ...
                    result += "<tr><th>" + "Gelen Çağrı" + "</th><td>" + dt.Rows[0]["ACDCALLS2"].ToString() + "</td></tr>";
                    result += "<tr><th>" + "Top. Çağrı S." + "</th><td>" + TimespanStr(dt.Rows[0]["TACDTIME2"].ToString()) + "</td></tr>";
                    result += "<tr><th>" + "Ort. Çağrı S." + "</th><td>" + TimespanStr(dt.Rows[0]["AACDTIME2"].ToString()) + "</td></tr>";
                    result += "<tr><th>" + "Bekletilen Çağrı" + "</th><td>" + dt.Rows[0]["HOLDCALLS2"].ToString() + "</td></tr>";
                   ...
                   // result += "<tr><th>" + "Sistem ACW" + "</th><td>" + TimespanStr(dt.Rows[0]["SYSTEMACW2"].ToString()) + " (" + DurationPercentage(dt.Rows[0]["SYSTEMACW2"].ToString(), dt.Rows[0]["TSTAFFTIME2"].ToString()) + "%)</td></tr>";
                    ...
                    result += "<tr><th>" + "Çalışma S." + "</th><td>" +TimespanStr( dt.Rows[0]["TSTAFFTIME2"].ToString()) + "</td></tr>";
                    result += "<tr><th>" + "Skill Bilgisi:" + "</th><td>" + LoadAgentSkills(Request.QueryString["agentId"].ToString()) +"</td></tr>";
