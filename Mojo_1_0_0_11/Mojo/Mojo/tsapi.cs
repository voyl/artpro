...
namespace Mojo
{
    public class Tsapi
    {
        ...
        public bool close()
        {
            ...
            try
            {
                int openStream = Csta.acsCloseStream(acsHandle, invokeId, ref privData);
            }
            catch (System.Exception eOpenStream)
            {
                // If we can't open the stream record the error and inform the user
        ...
        public bool monitor(string strExtension)
        {
            ...
            privData.vendor = "MERLIN                          ".ToCharArray();
            privData.length = Csta.CSTA_MAX_HEAP;
            privData.data = "N".ToCharArray();
            ...
            monitorFilter.call = 0xFFFF - Csta.CF_DELIVERED - Csta.CF_CONNECTION_CLEARED - Csta.CF_CALL_CLEARED - Csta.CF_ORIGINATED - Csta.CF_ESTABLISHED - Csta.CF_NETWORK_REACHED - Csta.CF_TRANSFERRED - Csta.CSTA_SET_AGENT_STATE_CONF;    // Monitor these call events 
            monitorFilter.feature = 0;                                                                                                  // Monitor everything feature-wise
            monitorFilter.agent = 1;                                                                                                    // Monitor everything agent-wise
        ...
        // The public method to monitor the provided extension
        public bool monitorVDN(string strExtension)
        {
            // Convert the extension string to a character array
            chExtension = strExtension.ToCharArray();
            ...
            // Any filters NOT added will allow those events to be monitored
            Csta.CSTAMonitorFilter_t monitorFilter = new Csta.CSTAMonitorFilter_t();
            monitorFilter.call = 0xFFFF - Csta.CF_DELIVERED - Csta.CF_CONNECTION_CLEARED - Csta.CF_CALL_CLEARED - Csta.CF_ORIGINATED - Csta.CF_ESTABLISHED - Csta.CF_NETWORK_REACHED -  Csta.CF_TRANSFERRED;    // Monitor these call events 
            monitorFilter.feature = 0;                                                                                                  // Monitor everything feature-wise
            monitorFilter.agent = 1;                                                                                                    // Monitor everything agent-wise
            monitorFilter.maintenance = 0;                                                                                              // Monitor everything maintenance-wise
            monitorFilter.privateFilter = 1;                                                                                            // Mandatory but unused
            ...
            try
            {
                int monitorDevice = Csta.cstaMonitorCallsViaDevice(acsHandle, numInvokeId, chExtension, ref monitorFilter, ref privData);                
        ...
        private void isRinging()
        {
            ...
            //MessageBox.Show(eventBuf.data.Length.ToString());
        }
        ...
        private void isDialed()
        {
            ...
            activeConnectionDeviceId = connectionDeviceId;
            activeConnectionDeviceIdType = connectionDeviceIdType.ToString();*/
            lastDisconectedCallId = connectionCallId.ToString();
        }
        ...
        public  void TsHandler(object sender, TServerBufferEventArgs e)
        {
            //Csta.CSTA_SET_AGENT_STATE
            // Ringing call
            ...
            // Message waiting indicator update
            else if (e.EventClass == Csta.CSTACONFIRMATION && e.EventType == Csta.CSTA_SET_AGENT_STATE_CONF)
            {
                mwiStatus();
            }
        ...
        public void holdCall(uint callId, char[] device, uint deviceType)
        {
            ...
            activeCall.devIDType = Csta.ConnectionID_Device_t.STATIC_ID;//(Csta.ConnectionID_Device_t)deviceType;
        ...
        public void getAgentState(uint callId, char[] device, uint deviceType)
        {
            // Define the mandatory (unused) private data buffer
            Csta.PrivateData_t privData = new Csta.PrivateData_t();
            privData.vendor = "MERLIN                          ".ToCharArray();
            ...
            int agentState = Csta.cstaQueryAgentState(acsHandle, numInvokeId, activeDeviceId, ref privData);
        }
        ...
        public void hangupCall(uint callId, char[] device, uint deviceType)
        {
            ...
            int clearConnection = Csta.cstaClearCall(acsHandle, numInvokeId, ref activeCall, ref privData);
        }

        // The public method to transfer an active call
        public void transferCall(uint heldCallId, char[] heldCallDevice, uint heldDeviceType,
            uint callId, char[] callDevice, uint deviceType)
        {
            // Define the mandatory (unused) private data buffer
            Csta.PrivateData_t privData = new Csta.PrivateData_t();
            privData.vendor = "MERLIN                          ".ToCharArray();
            ...
            // Populate a ConnectionID_t struct with the active call elements
            Csta.ConnectionID_t heldCall = new Csta.ConnectionID_t();
            heldCall.callID = heldCallId;
            heldCall.deviceID.device = heldCallDevice;
            heldCall.devIDType = Csta.ConnectionID_Device_t.STATIC_ID;

            Csta.ConnectionID_t activeCall = new Csta.ConnectionID_t();
            activeCall.callID = callId ;
            activeCall.deviceID.device = heldCallDevice;
            activeCall.devIDType = Csta.ConnectionID_Device_t.STATIC_ID;


            int clearConnection = Csta.cstaTransferCall(acsHandle, numInvokeId, ref heldCall, ref activeCall, ref privData);
        }
        ...
        public void SetAgentState(string agentId, string agentPwd, Csta.AgentMode_t state, string AUXCode)
        {
            ...
            // Populate a ConnectionID_t struct with the active call elements
          //  int invokeId = Csta.cstaSetAgentState(acsHandle, numInvokeId, chExtension, state, "".ToCharArray(), "".ToCharArray(), "".ToCharArray(), ref privData);
            
            
        }

        public int CstaSetAgentState(string device, string agentID, string agentPassword, Csta.AgentMode_t attWorkMode, Csta.AgentMode_t workMode, int reasonCode)
        {
            Csta.PrivateData_t _privData = new Csta.PrivateData_t();
           // IntPtr _ptrPrivData = Marshal.AllocHGlobal(Marshal.SizeOf(_privData));
           // Marshal.StructureToPtr(_privData, _ptrPrivData, true);

            _privData.vendor = "MERLIN                          ".ToCharArray();
            _privData.length = 4;
            _privData.data = "N".ToCharArray();


            char[] _agentID = "".ToCharArray();
            char[] _group = "".ToCharArray();
            char[] _agentPWd = "".ToCharArray();
          //  int _retPriv = Csta.attV6SetAgentState(ref _privData, (int)attWorkMode, reasonCode, true);

            int _rc = Csta.cstaSetAgentState(acsHandle, numInvokeId, ref chExtension, Csta.AgentMode_t.AM_READY, ref _agentID , ref _group, ref _agentPWd, null);

           // Marshal.FreeHGlobal(_ptrPrivData);
        ...
        public Csta.PrivateData_t privData = new Csta.PrivateData_t();
        ...
        Csta.EventBuf_t eventBuf = new Csta.EventBuf_t();
        public UInt32 activeCallId;
        public char[] activeDeviceId;
        public int activeDeviceIdType;
        public string lastDisconectedCallId;
        ...
        public int calledDeviceIdType;
        public int calledDeviceIdStatus;
        public string calledDeviceId;
        public string strLastDeviceId;
